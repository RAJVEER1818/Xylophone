...
#import "RunnerTestsParameterAddresses.h"

constexpr uint16_t kMaxVelocity = std::numeric_limits<std::uint16_t>::max();

/*
 RunnerTestsDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class RunnerTestsDSPKernel {
public:
    ...
    // Add a case for each parameter in RunnerTestsParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case RunnerTestsParameterAddress::midiNoteNumber:
                mNextNoteToSend = (uint8_t)value;
                break;
            case RunnerTestsParameterAddress::sendNote:
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        /*
         ...
         auto thisObject = static_cast<RunnerTestsDSPKernel *>(context);
